package farmSystem.closeUp.config.oauth2.handler;

import farmSystem.closeUp.config.jwt.JwtService;
import farmSystem.closeUp.config.oauth2.CustomOAuth2User;
    ...
    public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response, Authentication authentication) throws IOException, ServletException {
        ...
        try {
            ...
            if(oAuth2User.getUserRole() == UserRole.GUEST) {
                String accessToken = jwtService.createAccessToken(oAuth2User.getUserId(), oAuth2User.getEmail().toString());
                response.addHeader(jwtService.getAccessHeader(), "Bearer " + accessToken);
                response.sendRedirect("/oauth2/sign-up"); // 프론트의 회원가입 추가 정보 입력 폼으로 리다이렉트(추후 주소 변경)
...
//                findUser.authorizeUser();
    ...
    private void loginSuccess(HttpServletResponse response, CustomOAuth2User oAuth2User) throws IOException {
        String accessToken = jwtService.createAccessToken(oAuth2User.getUserId(), oAuth2User.getEmail());
